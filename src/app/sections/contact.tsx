  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        setStatus("ok");
        setMessage("");
        // (optionnel) reset du nom/email si tu veux :
        // setName(""); setEmail("");
      } else {
        setStatus("error");
      }
    ...
    >
      <div className="mx-auto max-w-5xl px-4">
        ...
        <div className="text-center">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-white">CONTACT</h2>
          <h2 className="mt-3 text-3xl md:text-5xl font-extrabold tracking-tight text-white">
            Prêt à développer<br className="hidden sm:block" /> votre prochain projet ?
          </h2>
          <p className=" mt-4 text-sm md:text-base text-white/70 max-w-2xl mx-auto">
            Prise de contact, témoignage ou simple bonjour… Laissez‑moi un message et vos coordonnées si
        ...
        <GlassBlock variant="base" className="mt-10 p-6 md:p-8">
          <form onSubmit={onSubmit} className="grid gap-4 md:gap-5">
            {/* Honey pot (caché des users, visible des bots) */}
            <input
              type="text"
              value={hp}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label className="grid gap-1">
                <span className="text-xs text-white/70">Votre nom</span>
                <input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Phillipe Auguste II"
              ...
              {/* Email */}
              <label className="grid gap-1">
                {/* En-tête sur une ligne, hauteur fixe */}
                <div className="flex items-center justify-between h-5">
                  <span className="text-xs text-white/70">Votre e-mail</span>
                  <span
                    className={`text-[11px] ${
                      emailError ? "text-red-400" : "invisible"
                    }`}
                    aria-live="polite"
                    id="email-help"
                  >
                ...
                <input
                  ...
                  className={`h-11.5 rounded-xl bg-white/5 text-white placeholder-white/40 border px-3 py-2 outline-none ring-1 ring-inset
                    ${emailError ? "border-white/0 ring-red-400/60 focus:ring-red-400"
            ...
            {/* Message */}
            <label className="grid gap-1">
              <span className="text-xs text-white/70">Votre message</span>
              <textarea
                value={message}
                ...
                required
              />
              <div className="text-right text-xs text-white/50">{message.length} caractères</div>
            </label>
            ...
            <div className="mt-2 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 text-sm">
                <Link /* ... */ href="https://www.linkedin.com/in/thibault-cauche-aa2983180/" target="_blank"
                ...
                <Link /* ... */ href="https://discord.com/users/thibault9090" target="_blank"
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-white/80 hover:text-white">
              ...
              {/* Submit (même style que “CV hover”) */}
              <button
                type="submit"
